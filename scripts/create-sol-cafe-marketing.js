...
/**
 * Script để tạo project "sol-cafe-coworking-mkt" và 30 marketing tasks
 * Sử dụng internal SDK thay vì HTTP API
 */

const { initDb } = require('/home/roxane/projects/claude-ws/packages/agentic-sdk/src/db');
const { createProjectService } = require('/home/roxane/projects/claude-ws/packages/agentic-sdk/src/services/project/project-crud');
const { createTaskService } = require('/home/roxane/projects/claude-ws/packages/agentic-sdk/src/services/task/task-crud-and-reorder');
const { createAttemptService } = require('/home/roxane/projects/claude-ws/packages/agentic-sdk/src/services/attempt/attempt-crud-and-logs');
...
const MARKETING_TASKS = [
  ...
  },
  {
    ...
    description: "Lên kế hoạch campaign kỷ niệm 1 năm: Theme ideas, special promotions (flash sales, upgrade deals), member appreciation events, social media contest, user-generated content campaign, press outreach, timeline 4 weeks before và after."
  }
];
...
async function main() {
  ...
  const db = initDb(path.join(process.cwd(), 'data'));
  ...
  console.log('📁 Step 1: Creating project "sol-cafe-coworking-mkt"...');
  try {
    ...
    console.log('📝 Step 2: Creating 30 marketing tasks...\n');
    ...
    for (let i = 0; i < MARKETING_TASKS.length; i++) {
      ...
      try {
        ...
        tasks.push(task);
        console.log(`  ✓ Task ${i + 1}/30: ${task.title.substring(0, 50)}... (ID: ${task.id})`);
      } catch (error) {
        console.error(`  ✗ Failed to create task ${i + 1}: ${error.message}`);
      }
    }

    console.log(`\n✓ Created ${tasks.length}/30 tasks\n`);

    // Step 3: Create attempts for each task
    console.log('🚀 Step 3: Creating attempts for each task...\n');
...
Hãy bắt đầu với deliverable chất lượng cao, ready để implement!`;
      ...
      try {
        ...
        console.log(`  ✓ Attempt ${i + 1}/30 created (ID: ${attempt.id})`);
      } catch (error) {
        console.error(`  ✗ Failed to create attempt for task ${i + 1}: ${error.message}`);
      }
    }

    console.log(`\n✓ Created ${attempts.length}/30 attempts\n`);
    ...
    console.log(`✓ Tasks Created: ${tasks.length}/30`);
    console.log(`✓ Attempts Created: ${attempts.length}/30`);
    console.log('\n🎉 Setup complete! The AI agents are now working on your marketing tasks.');
    console.log('\n📊 Monitor progress:');
    console.log(`   - View tasks: curl http://localhost:33333/api/tasks?projectId=${project.id}`);
    console.log(`   - View attempts: curl http://localhost:33333/api/attempts?projectId=${project.id}`);
    console.log(`   - Check database: sqlite3 data/db.sqlite "SELECT * FROM attempts WHERE task_id IN (SELECT id FROM tasks WHERE project_id = '${project.id}');"`);

  } catch (error) {
    if (error.message.includes('UNIQUE constraint failed')) {
      console.log('ℹ️  Project "sol-cafe-coworking-mkt" already exists.');
      console.log('   Skipping project creation. To recreate, delete the existing project first.');
    } else {
